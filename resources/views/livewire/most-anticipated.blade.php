<div wire:init="loadMostAnticipated" class="most-anticipated-container space-y-10 mt-8">
    ...
    <div class="game flex">
        <a href="{{ route('games.show', $anticipated['slug']) }}">
            <img src="{{ $anticipated['coverImageUrl'] }}"
                 class="w-16 hover:opacity-75 transition ease-in-out duration-150"
                 alt="game cover for {{ $anticipated['name'] }}">
        </a>
        <div class="ml-4">
            <a href="{{ route('games.show', $anticipated['slug']) }}">{{ $anticipated['name'] }}</a>
            <div class="text-gray-400 text-sm mt-1">
                {{ $anticipated['releaseDate'] }}
            </div>
        </div>
    </div>
    @empty
